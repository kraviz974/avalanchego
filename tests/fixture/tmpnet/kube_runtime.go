...
func (p *KubeRuntime) setNotRunning() {
	p.node.network.log.Trace("node is not running",
...
func (p *KubeRuntime) readState(ctx context.Context) error {
	...
	log.Trace("checking if StatefulSet exists",
	...
	if apierrors.IsNotFound(err) {
		log.Trace("StatefulSet not found",
			zap.Stringer("nodeID", nodeID),
			zap.String("namespace", namespace),
			zap.String("statefulSet", statefulSetName),
		)
	...
	if scale.Spec.Replicas == 0 {
		log.Trace("StatefulSet has no replicas",
			zap.Stringer("nodeID", nodeID),
			zap.String("namespace", namespace),
			zap.String("statefulset", statefulSetName),
		)
	...
	if err := p.waitForPodReadiness(ctx); err != nil {
		log.Trace("failed to wait for Pod readiness",
			zap.Stringer("nodeID", nodeID),
			zap.String("namespace", namespace),
			zap.String("statefulSet", statefulSetName),
			zap.Error(err),
...
func (p *KubeRuntime) Start(ctx context.Context) error {
	...
	log.Info("attempting to retrieve existing StatefulSet",
		zap.Stringer("nodeID", nodeID),
		zap.String("namespace", namespace),
		zap.String("statefulSet", statefulSetName),
	)
	...
	if exists {
		log.Info("attempting to retrieve scale for existing StatefulSet",
			zap.Stringer("nodeID", nodeID),
			zap.String("namespace", namespace),
			zap.String("statefulSet", statefulSetName),
		)
		...
		if scale.Spec.Replicas != 0 {
			log.Info("StatefulSet is already running",
				zap.Stringer("nodeID", nodeID),
				zap.String("namespace", namespace),
				zap.String("statefulSet", statefulSetName),
			)
		...
		log.Info("attempting to scale up StatefulSet",
			zap.Stringer("nodeID", nodeID),
			zap.String("namespace", namespace),
			zap.String("statefulSet", statefulSetName),
		)
		...
		log.Info("scaled up StatefulSet",
			zap.Stringer("nodeID", nodeID),
			zap.String("namespace", namespace),
			zap.String("statefulSet", statefulSetName),
		)
...
func (p *KubeRuntime) ensureBootstrapIP(ctx context.Context) error {
	...
	if len(bootstrapIPs) > 0 {
		log.Info("bootstrap IPs are already available so no need to wait for StatefulSet Pod to become ready",
...
func (p *KubeRuntime) waitForPodReadiness(ctx context.Context) error {
	...
	log.Trace("waiting for Pod to become ready",
		zap.Stringer("nodeID", nodeID),
		zap.String("namespace", namespace),
	...
	log.Trace("pod is ready",
		zap.Stringer("nodeID", nodeID),
		zap.String("namespace", namespace),
		zap.String("pod", podName),
	)

	log.Trace("retrieving Pod IP",
		zap.Stringer("nodeID", nodeID),
		zap.String("namespace", namespace),
		zap.String("pod", podName),
	)
...
func (p *KubeRuntime) InitiateStop(ctx context.Context) error {
	...
	log.Info("retrieving StatefulSet scale",
		zap.Stringer("nodeID", nodeID),
		zap.String("namespace", namespace),
		zap.String("statefulSet", statefulSetName),
	)
	...
	log.Info("setting StatefulSet replicas to zero",
		zap.Stringer("nodeID", nodeID),
		zap.String("namespace", namespace),
		zap.String("statefulSet", statefulSetName),
	)
	...
	log.Info("StatefulSet replicas set to zero",
		zap.Stringer("nodeID", nodeID),
		zap.String("namespace", namespace),
		zap.String("statefulSet", statefulSetName),
	)
...
func (p *KubeRuntime) WaitForStopped(ctx context.Context) error {
	...
	err = wait.PollUntilContextCancel(
		...
		func(ctx context.Context) (bool, error) {
			scale, err := clientset.AppsV1().StatefulSets(namespace).GetScale(ctx, statefulSetName, metav1.GetOptions{})
			if apierrors.IsNotFound(err) {
				log.Info("node stopped: StatefulSet not found",
					zap.Stringer("nodeID", nodeID),
					zap.String("namespace", namespace),
					zap.String("statefulSet", statefulSetName),
				)
			...
			if scale.Status.Replicas == 0 {
				log.Info("node stopped: StatefulSet scaled to zero replicas",
					zap.Stringer("nodeID", nodeID),
					zap.String("namespace", namespace),
					zap.String("statefulSet", statefulSetName),
				)
...
func (p *KubeRuntime) Restart(ctx context.Context) error {
	...
	log.Debug("initiating StatefulSet restart",
		zap.String("nodeID", nodeID),
		zap.String("namespace", namespace),
		zap.String("statefulSet", statefulSetName),
	)
...
func (p *KubeRuntime) GetLocalStakingAddress(ctx context.Context) (netip.AddrPort, func(), error) {
	if p.node.StakingAddress == (netip.AddrPort{}) {
		// Assume that an empty URI indicates a need to retrieve pod state
		if err := p.readState(ctx); err != nil {
